...
class NepaliSummarizer:
    ...
    def calculate_keyword_score(self, sentence):
        ...
        for word in words:
            if word in self.important_words:
                score += 1.5
            elif word not in self.nepali_stopwords and len(word) > 2:
                score += 1.0
            else:
                score += 0.2
    ...
    def evaluate_on_dataset(self, dataset_path, text_column='cleaned_text', summary_column='cleaned_summary', num_sentences=5, sample_size=None):
        ...
        if 'cleaned_text' not in df.columns:
            ...
            df['cleaned_text'] = df['article'].apply(trainer.clean_dataset_text)
        
        if 'cleaned_summary' not in df.columns:
            print("Creating cleaned_summary column...")
            df['cleaned_summary'] = df['summary'].apply(trainer.clean_dataset_text)
